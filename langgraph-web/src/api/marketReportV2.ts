...
const GENERATE_URL = '/api/report-v2'
...
async function readStreamBodyV2(
...
export async function generateMarketReportV2(
    ...
    // 始终按 SSE 格式解析，不依赖 content-type 头
    // （Vite 代理在某些情况下可能不转发 text/event-stream 头）
    await readStreamBodyV2(reader, true, options)
}
